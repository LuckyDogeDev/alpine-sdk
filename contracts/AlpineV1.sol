...
contract MasterContractManager is BoringOwnable, BoringFactory {
    ...
    constructor() public {
        ...
        DOMAIN_SEPARATOR = keccak256(abi.encode(DOMAIN_SEPARATOR_SIGNATURE_HASH, "Alpine V2", chainId, address(this)));
    }
                    ...
                    abi.encodePacked(
                        ...
                        keccak256(
                            abi.encode(
                                ...
                                approved ? "Give FULL access to funds in (and approved to) Alpine?" : "Revoke access to Alpine?",
                                user,
                                masterContract,
                                approved,
                                nonces[user]++
...
// File contracts/Alpine.sol
// License-Identifier: UNLICENSED

/// @title Alpine
/// @author BoringCrypto, Keno
/// @notice The Alpine is a vault for tokens. The stored tokens can be flash loaned and used in strategies.
/// Yield from this will go to the token depositors.
/// Rebasing tokens ARE NOT supported and WILL cause loss of funds.
/// Any funds transfered directly onto the Alpine will be lost, use the deposit function instead.
contract AlpineV1 is MasterContractManager, BoringBatchable {
    using BoringMath for uint256;
    using BoringMath128 for uint128;
    ...
    struct StrategyData {
        ...
        uint128 balance; // the balance of the strategy that Alpine thinks is in there
    }
    ...
    // If 'from' is the Alpine itself, it's allowed. Any ETH, token balances (above the known balances) or Alpine balances
    ...
    modifier allowed(address from) {
        if (from != msg.sender && from != address(this)) {
            ...
            require(masterContract != address(0), "Alpine: no masterContract");
            require(masterContractApproved[masterContract][from], "Alpine: Transfer not approved");
        }
        _;
    }
    ...
    function deposit(
        ...
        // Checks
        require(to != address(0), "Alpine: to not set"); // To avoid a bad UI from burning funds
        ...
        require(total.elastic != 0 || token.totalSupply() > 0, "Alpine: No tokens");
        if (share == 0) {
    ...
    function withdraw(
        ...
        require(total.base >= MINIMUM_SHARE_BALANCE || total.base == 0, "Alpine: cannot empty");
        totals[token] = total;
        ...
        if (token_ == USE_ETHEREUM) {
            ...
            require(success, "Alpine: ETH transfer failed");
        } else {
    ...
    function transferMultiple(
        ...
        // Checks
        require(tos[0] != address(0), "Alpine: to[0] not set"); // To avoid a bad UI from burning funds
    ...
    function flashLoan(
        ...
        require(_tokenBalanceOf(token) >= totals[token].addElastic(fee.to128()), "Alpine: Wrong amount");
    ...
    function batchFlashLoan(
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            require(_tokenBalanceOf(token) >= totals[token].addElastic(fees[i].to128()), "Alpine: Wrong amount");
    ...
    // F5 - Checks-Effects-Interactions pattern followed? (SWC-107)
    // F5: Total amount is updated AFTER interaction. But strategy is under our control.
    // F5: Not followed to prevent reentrancy issues with flashloans and Alpine skims?
    function harvest(
        IERC20 token,
        bool balance,
        ...
        if (balanceChange > 0) {
            ...
            // But tokens with balances that large are not supported by the Alpine.
            uint256 sub = uint256(-balanceChange);
